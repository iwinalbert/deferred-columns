...
unique_ptr<GlobalTableFunctionState> DeferredScanInit(ClientContext &context, TableFunctionInitInput &input) {
	...
	if (input.filters) {
		...
		for (auto &pair : input.filters->filters) {
			...
			string col_name;
			if (absolute_col_idx == 0)
				col_name = "category";
			else if (absolute_col_idx == 1)
				col_name = "date";
			else if (absolute_col_idx == 2)
				col_name = "f1";
			else if (absolute_col_idx == 3)
				col_name = "f2";
			else if (absolute_col_idx == 4)
				col_name = "f3";
			else if (absolute_col_idx == 5)
				col_name = "rmse";
...
unique_ptr<LocalTableFunctionState> DeferredScanInitLocal(ExecutionContext &context, TableFunctionInitInput &input,
...
void DeferredScanFunction(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
	...
	if (state.session && state.model_name != "mock_model" && !valid_indices.empty()) {
		...
		auto output_tensors =
		    state.session->Run(Ort::RunOptions {nullptr}, input_names, &input_tensor, 1, output_names, 1);
