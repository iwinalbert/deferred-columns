...
extern unique_ptr<LocalTableFunctionState>
DeferredScanInitLocal(ExecutionContext &context, TableFunctionInitInput &input, GlobalTableFunctionState *global_state);
...
static void LoadInternal(ExtensionLoader &loader) {
	...
	TableFunction tf_1({LogicalType::VARCHAR}, DeferredScanFunction, DeferredScanBind, DeferredScanInit,
	                   DeferredScanInitLocal);
	tf_1.filter_pushdown = true;
	tf_1.filter_prune = true;
	deferred_scan.AddFunction(tf_1);

	TableFunction tf_2({LogicalType::VARCHAR, LogicalType::BIGINT}, DeferredScanFunction, DeferredScanBind,
	                   DeferredScanInit, DeferredScanInitLocal);
	...
	swap_model_set.AddFunction(
	    ScalarFunction({LogicalType::INTEGER, LogicalType::VARCHAR}, LogicalType::BOOLEAN, SwapModelFunction));
